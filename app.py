...
# -------------------------
    ...
    if st.button("Run Batch Prediction") and (multi is not None and len(multi) > 0):
        results = []
        for up in multi:
            tmpname = f"batch_{int(time.time()*1000)}_{up.name}"
        ...
        if st.button("Load selected"):
            ...
            if st.button("Export doctor summary (PDF)"):
                if REPORTLAB_AVAILABLE:
                    ...
                    c.drawString(40, height - 60, "Doctor Summary — Automated Brain Stroke Classification")
                    c.setFont("Helvetica", 11)
                    y = height - 90
                    ...
                    y -= 16
                    c.setFont("Helvetica", 11)
                    text_lines = str(notes).splitlines()
...
# -------------------------
    ...
    st.header("Settings")
    st.markdown("Model & app settings. If model failed to load, re-check this path and model file format (state_dict required).")
    ...
    if os.path.exists(LOGS_CSV):
        if st.button("Download logs CSV"):
            with open(LOGS_CSV, "rb") as f:
                st.download_button("Download", f, file_name="predictions_log.csv")
